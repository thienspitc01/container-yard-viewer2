...
// Helper to calculate TEU locally matching App.tsx logic
const calculateTEU = (container: Container): number => {
...
const YardStatistics: React.FC<YardStatisticsProps> = ({ data, containers, blocks }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden min-h-[500px]" ref={stockOverviewRef}>
        <div 
          ...
          <div className="overflow-x-auto">
            ...
            <thead>
                ...
                <th className="border-r border-slate-300 p-3 bg-cyan-700 w-28 text-left align-bottom relative group cursor-pointer" ref={filterRef} onClick={() => setIsFilterOpen(!isFilterOpen)}>
                    ...
                    {/* Filter Dropdown */}
                    {isFilterOpen && (
                    <div className="absolute top-full left-0 mt-1 w-56 bg-white border border-slate-200 rounded-lg shadow-2xl z-50 text-slate-700 max-h-80 flex flex-col" onClick={(e) => e.stopPropagation()}>
                        ...
                        {allBlockNames.map((name: string) => (
                            <label key={name} className="flex items-center space-x-2 cursor-pointer hover:bg-cyan-50 p-2 rounded transition-colors">
                            <input 
                                type="checkbox" 
